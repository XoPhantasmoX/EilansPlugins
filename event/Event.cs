...
namespace EilansPlugins.Event
{
    ...
    {
        public List<EventPartBase> EventParts { get; set; }

        public EasingEvent(double timeLength, double initValue) => Init(timeLength, initValue);

        public EventPartBase this[int index] => EventParts[index];

        public IEnumerator GetEnumerator() => EventParts.GetEnumerator();

        public void Init(double timeLength, double initValue)
        {
            if (timeLength == 0) EventParts = new List<EventPartBase> { };
            else EventParts = new List<EventPartBase> { new EasingEventPartTail(0, initValue) };
        }
        ...
        public void Divede(double time)
        {
            ...
            EventPartBase[] dividedParts = EventParts[i].Divide(time);
        ...
        public EasingEvent Copy()
        {
            ...
            foreach (EventPartBase part in EventParts)
            {
